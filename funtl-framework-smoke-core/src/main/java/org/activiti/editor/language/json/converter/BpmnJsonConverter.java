...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
	...
	public ObjectNode convertToJson(BpmnModel model) {
		...
		//		modelNode.put("bounds", BpmnJsonConverterUtil.createBoundsNode(maxX, maxY, 0, 0)); // 已过时 - 2016-09-03 by Lusifer
		modelNode.put("resourceId", "canvas");

		ObjectNode stencilNode = objectMapper.createObjectNode();
		stencilNode.put("id", "BPMNDiagram");
		modelNode.set("stencil", stencilNode);
		//		modelNode.put("stencil", stencilNode); // 已过时 - 2016-09-03 by Lusifer
		...
		//		modelNode.put("stencilset", stencilsetNode); // 已过时 - 2016-09-03 by Lusifer
		...
		//		modelNode.put(EDITOR_SHAPE_PROPERTIES, propertiesNode); // 已过时 - 2016-09-03 by Lusifer
		...
		if ((model.getPools().size() > 0) && poolHasDI) {
			for (Pool pool : model.getPools()) {
				...
				//				poolNode.put(EDITOR_SHAPE_PROPERTIES, poolPropertiesNode); // 已过时 - 2016-09-03 by Lusifer

				ArrayNode laneShapesArrayNode = objectMapper.createArrayNode();
				poolNode.set(EDITOR_CHILD_SHAPES, laneShapesArrayNode);
				//				poolNode.put(EDITOR_CHILD_SHAPES, laneShapesArrayNode); // 已过时 - 2016-09-03 by Lusifer

				ArrayNode outgoingArrayNode = objectMapper.createArrayNode();
				poolNode.set("outgoing", outgoingArrayNode);
				//				poolNode.put("outgoing", outgoingArrayNode); // 已过时 - 2016-09-03 by Lusifer
				...
				if (process != null) {
					...
					for (Lane lane : process.getLanes()) {
						...
						//						laneNode.put(EDITOR_SHAPE_PROPERTIES, lanePropertiesNode); // 已过时 - 2016-09-03 by Lusifer

						ArrayNode elementShapesArrayNode = objectMapper.createArrayNode();
						laneNode.set(EDITOR_CHILD_SHAPES, elementShapesArrayNode);
						//						laneNode.put(EDITOR_CHILD_SHAPES, elementShapesArrayNode); // 已过时 - 2016-09-03 by Lusifer
						laneNode.set("outgoing", objectMapper.createArrayNode());
						//						laneNode.put("outgoing", objectMapper.createArrayNode()); // 已过时 - 2016-09-03 by Lusifer
		...
		//		modelNode.put(EDITOR_CHILD_SHAPES, shapesArrayNode); // 已过时 - 2016-09-03 by Lusifer
