...
import com.funtl.framework.smoke.core.commons.annotation.FieldName;
			...
			for (Method m : entity.getClass().getMethods()) {
				if (m.getAnnotation(JsonIgnore.class) == null && m.getAnnotation(JsonBackReference.class) == null && m.getName().startsWith("get")) {
					...
					if (m.getName().equals("getAct")) {
						...
						//						map.put("taskId", ObjectUtils.toString(m.invoke(act, new Object[]{}), ""));  // 已过时 - 2016-09-03 by Lusifer
					} else {
						field.add(StringUtils.uncapitalize(m.getName().substring(3)));
						value.add(Objects.toString(m.invoke(entity, new Object[]{}), ""));
						//						value.add(ObjectUtils.toString(m.invoke(entity, new Object[]{}), "")); // 已过时 - 2016-09-03 by Lusifer
					}
				}
			}
		} catch (Exception e) {
