...
package com.funtl.framework.smoke.core.modules.act.rest.diagram.services;
...
public class BaseProcessDefinitionDiagramLayoutResource {
	...
	public ObjectNode getDiagramNode(String processInstanceId, String processDefinitionId) {
		...
		if (pdrJSON != null) {
			...
			//			responseJSON.put("processDefinition", pdrJSON); // 已过时 - 2016-09-03 by Lusifer
		}
		...
		if (processInstance != null) {
			...
			//			responseJSON.put("highLightedActivities", activityArray); // 已过时 - 2016-09-03 by Lusifer
			//			responseJSON.put("highLightedFlows", flowsArray); // 已过时 - 2016-09-03 by Lusifer
		}
		...
		if (processDefinition.getParticipantProcess() != null) {
			...
			//			responseJSON.put("participantProcess", participantProcessJSON); // 已过时 - 2016-09-03 by Lusifer
		}
			...
			for (LaneSet laneSet : processDefinition.getLaneSets()) {
				...
				if (laneSet.getLanes() != null && !laneSet.getLanes().isEmpty()) {
					for (Lane lane : laneSet.getLanes()) {
						...
						//						laneJSON.put("flowNodeIds", flowNodeIdsArray); // 已过时 - 2016-09-03 by Lusifer
				...
				//				laneSetJSON.put("lanes", laneArray); // 已过时 - 2016-09-03 by Lusifer

				laneSetArray.add(laneSetJSON);
			}

			if (laneSetArray.size() > 0) responseJSON.set("laneSets", laneSetArray);
			//				responseJSON.put("laneSets", laneSetArray); // 已过时 - 2016-09-03 by Lusifer
		}
		...
		//		responseJSON.put("activities", activityArray); // 已过时 - 2016-09-03 by Lusifer
		//		responseJSON.put("sequenceFlows", sequenceFlowArray); // 已过时 - 2016-09-03 by Lusifer
	...
	private void getActivity(String processInstanceId, ActivityImpl activity, ArrayNode activityArray, ArrayNode sequenceFlowArray, ProcessInstance processInstance, List<String> highLightedFlows, Map<String, ObjectNode> subProcessInstanceMap) {
		...
		for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
			...
			//			flowJSON.put("xPointArray", xPointArray); // 已过时 - 2016-09-03 by Lusifer
			//			flowJSON.put("yPointArray", yPointArray); // 已过时 - 2016-09-03 by Lusifer
		...
		for (String key : properties.keySet()) {
			...
			if (prop instanceof String) propertiesJSON.put(key, (String) properties.get(key));
				...
				if (timerDeclarationArray.size() > 0) propertiesJSON.set(key, timerDeclarationArray);
				//					propertiesJSON.put(key, timerDeclarationArray); // 已过时 - 2016-09-03 by Lusifer
				...
				if (eventDefinitionsArray.size() > 0) propertiesJSON.set(key, eventDefinitionsArray);
				//					propertiesJSON.put(key, eventDefinitionsArray); // 已过时 - 2016-09-03 by Lusifer
				...
				if (errorEventDefinitionsArray.size() > 0) propertiesJSON.set(key, errorEventDefinitionsArray);
				//					propertiesJSON.put(key, errorEventDefinitionsArray); // 已过时 - 2016-09-03 by Lusifer
			}
		...
		if ("callActivity".equals(properties.get("type"))) {
			...
			if (callActivityBehavior != null) {
				...
				if (processInstanceArray.size() > 0) {
					...
					//					propertiesJSON.put("processDefinitons", processInstanceArray); // 已过时 - 2016-09-03 by Lusifer
				}
			}
		}
		...
		//		activityJSON.put("properties", propertiesJSON); // 已过时 - 2016-09-03 by Lusifer
		if (multiInstance != null) activityJSON.put("multiInstance", multiInstance);
		if (collapsed) activityJSON.put("collapsed", collapsed);
		if (nestedActivityArray.size() > 0) activityJSON.set("nestedActivities", nestedActivityArray);
		//			activityJSON.put("nestedActivities", nestedActivityArray); // 已过时 - 2016-09-03 by Lusifer
